...
nix build --no-link ./hosts/nixos#nixosConfigurations.nixos.config.system.build.toplevel

# Optional: Dry run (read-only)
nixos-rebuild dry-run --flake ./hosts/nixos#nixos
```
...
✅ Enabled in `hosts/nixos/configuration.nix`
